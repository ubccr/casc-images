...
else if ( UPLOAD_ERR_INI_SIZE == $_FILES['imagefile']['error'] )
{
  $response = array('success' => false,
                    'msg'     => "Image exceeded maximim size of " . ini_get('upload_max_filesize'));
}
else if ( 0 !== strpos($_FILES['imagefile']['type'], "image") )
{
  $response = array('success' => false,
